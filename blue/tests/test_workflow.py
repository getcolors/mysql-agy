...
from package_mysql_agy_blue import tools, validate, workflow
from test_tools import recorded


def test_application_fork_join_and_cleanup_order_are_preserved():
    assert workflow.wire_fn('mysql-agy/ansible-local', {'blue/event': 'create'})[1:] == ('mysql-agy/dns', 'mysql-agy/base')
    assert workflow.wire_fn('mysql-agy/dns', {'blue/event': 'create'})[1] == 'mysql-agy/cluster'
    assert workflow.wire_fn('mysql-agy/base', {'blue/event': 'create'})[1] == 'mysql-agy/cluster'
    assert workflow.wire_fn('mysql-agy/dns', {'blue/event': 'delete'})[1] == 'mysql-agy/infrastructure'
    assert len(workflow.wire_fn('mysql-agy/infrastructure', {'blue/event': 'delete'})) == 1
...
async def test_native_build_renders_shared_endpoint_compute_and_application_agents(tmp_path):
    for load in (fixture, optout):
        ...
        result = await run(workflow.mysql_agy_workflow, load({'blue/event': 'build', 'workdir': str(directory)}))
