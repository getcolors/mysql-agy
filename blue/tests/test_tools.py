...
from package_mysql_agy_blue import compute, tools
...
def test_application_requires_endpoint_capability_and_legacy_migration_guard():
    ...
    assert policy['legacy_state_keys'] == ['mysql-agy-fixture/mysql-agy-infrastructure.tfstate']
...
def test_endpoint_assignment_and_health_delegate_to_library_agent():
    ...
    endpoint = (source / 'files/mysql-agy-endpoint').read_text()
    ...
    health = (source / 'files/mysql-agy-health').read_text()
