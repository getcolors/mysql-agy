...
from package_mysql_agy_blue import ssh_config

opts = fixture({"profile": "mysql-agy-digitalocean"})
...
def test_the_deployment_claims_one_alias_per_member_and_the_bare_profile():
    ...
    assert ssh_config.aliases(opts) == [
        "mysql-agy-digitalocean", "mysql-agy-digitalocean-0",
        "mysql-agy-digitalocean-1", "mysql-agy-digitalocean-2"]


def test_the_identity_file_stays_unexpanded():
    assert ssh_config.identity_file(opts) == "~/.ssh/mysql-agy-digitalocean"


def test_a_foreign_stanza_is_found_for_any_alias_not_just_the_first():
    lines = ("Host something\n  HostName 1.2.3.4\n\n"
             "Host mysql-agy-digitalocean-2\n  HostName 5.6.7.8\n").splitlines()
    assert ssh_config.foreign_stanza_line(lines, "mysql-agy-digitalocean") is None
    assert ssh_config.foreign_stanza_line(lines, "mysql-agy-digitalocean-2") == 4
...
def test_our_own_managed_block_is_not_foreign_for_any_alias_in_it():
    ...
    lines = ("# BEGIN mysql-agy-digitalocean ANSIBLE MANAGED BLOCK\n"
    ...
    for alias in ssh_config.aliases(opts):
        assert ssh_config.foreign_stanza_line(lines, alias, "mysql-agy-digitalocean") is None, alias
...
def test_a_member_stanza_outside_our_block_is_still_foreign():
    lines = ("# BEGIN mysql-agy-digitalocean ANSIBLE MANAGED BLOCK\n"
             ...
             "# END mysql-agy-digitalocean ANSIBLE MANAGED BLOCK\n"
             "Host mysql-agy-digitalocean-1\n  HostName 9.9.9.9\n").splitlines()
    assert ssh_config.foreign_stanza_line(lines, "mysql-agy-digitalocean-1", "mysql-agy-digitalocean") == 5
...
def test_the_refusal_is_reported_as_a_failed_step(monkeypatch, tmp_path):
    ...
    config.write_text("Host mysql-agy-digitalocean-1\n  HostName 9.9.9.9\n")
    monkeypatch.setenv("HOME", str(tmp_path))
    refused = ssh_config.preflight(opts)
    assert refused["blue/exit"] == 1
    assert "mysql-agy-digitalocean-1" in refused["blue/err"]
