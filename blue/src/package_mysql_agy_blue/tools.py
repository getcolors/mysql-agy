...
ansible_local_tool = "mysql-agy-ansible-local"
...
def output_params(result: dict) -> dict | None:
    ...
    apply reported none."""
    return cluster.output_params({"tofu/outputs": result.get("mysql-agy/outputs")})
...
async def load_infrastructure_step(opts: dict) -> dict:
    ...
    under `mysql-agy/state`, or a fresh read when nothing was. An unreadable
    ...
    if "mysql-agy/state" in opts:
        state = opts["mysql-agy/state"]
    else:
        state = await cluster.read_state(opts, state_output)
    handed = {k: v for k, v in opts.items() if k != "mysql-agy/state"}
    ...
    if failed(checked):
        return checked
    return {**checked, "mysql-agy/infrastructure-present?": present}
...
async def ansible_local_step(opts: dict) -> dict:
    ...
    if delete and opts.get("mysql-agy/infrastructure-present?") is False:
