...
(def ansible-local-tool "mysql-agy-ansible-local")
...
(defn output-params
  ...
  [result]
  (cluster/output-params {:tofu/outputs (:mysql-agy/outputs result)}))
...
(defn load-infrastructure-step
  ...
  `:mysql-agy/state`, or a fresh read when nothing was. An unreadable backend
  ...
  (let [event (:green/event opts)
        state (or (:mysql-agy/state opts) (cluster/read-state opts state-output))
        adopted (cluster/adopt-state validate/spec (dissoc opts :mysql-agy/state) event state)
    ...
    (cond
      ...
      :else (let [checked (checked adopted)]
              (if (wf/failed? checked)
                checked
                (assoc checked :mysql-agy/infrastructure-present? present?))))))
...
(defn ansible-local-step
  ...
  (if (and (= :delete (:green/event opts))
           (false? (:mysql-agy/infrastructure-present? opts)))
