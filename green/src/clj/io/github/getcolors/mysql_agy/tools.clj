(ns io.github.getcolors.mysql-agy.tools
  ...
  (:require [cheshire.core :as json]
            ...
            [io.github.getcolors.mysql-agy.compute :as compute]
...
(defn load-infrastructure-step [opts]
  (if (or (= :build (:green/event opts)) (:green/dry-run opts)) (infrastructure-step opts)
      (let [result (inspection/read-deployment opts)]
        (case (:status result)
          "destroyed" (if (= :delete (:green/event opts)) (assoc opts :mysql-agy/already-destroyed true :green/exit 0) (refuse opts ["compute inventory unavailable"]))
          "present" (cond-> (assoc opts :colors-compute/cluster (:cluster result) :colors-compute/shared (:shared result) :mysql-agy/infrastructure-present? true :green/exit 0)
