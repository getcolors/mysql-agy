(ns io.github.getcolors.mysql-agy.ssh-test
...
(deftest a-build-never-names-the-operators-home
  ...
  (let [opts (ssh/with-machine-key (assoc fixture :green/event :build))]
    (is (= "/home/build-placeholder/.ssh/mysql-agy-fixture" (:ssh-private-key-path opts)))
    (is (= "/home/build-placeholder/.ssh/mysql-agy-fixture.pub" (:ssh-public-key-path opts)))
    (testing "the placeholder lands on the provider's own machine-key key"
      (is (= "/home/build-placeholder/.ssh/mysql-agy-fixture.pub" (:digitalocean-ssh-keys opts))))
...
(deftest real-events-render-the-real-path
  (let [opts (ssh/with-machine-key (assoc fixture :green/event :health))]
    ...
    (is (.endsWith ^String (:ssh-private-key-path opts) "/.ssh/mysql-agy-fixture"))))
