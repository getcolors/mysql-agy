(ns io.github.getcolors.mysql-agy.ssh-config-test
  (:require [clojure.string :as str]
            [clojure.test :refer [deftest is testing]]
            [io.github.getcolors.mysql-agy.ssh-config :as ssh-config]))

(def opts {:profile "mysql-agy-digitalocean" :cluster-nodes 3})
...
(deftest the-deployment-claims-one-alias-per-member-and-the-bare-profile
  ...
  (is (= ["mysql-agy-digitalocean" "mysql-agy-digitalocean-0"
          "mysql-agy-digitalocean-1" "mysql-agy-digitalocean-2"]
         (ssh-config/aliases opts))))

(deftest the-identity-file-stays-unexpanded
  (is (= "~/.ssh/mysql-agy-digitalocean" (ssh-config/identity-file opts))))
...
(deftest a-foreign-stanza-is-found-for-any-alias-not-just-the-first
  (let [lines (str/split-lines "Host something\n  HostName 1.2.3.4\n\nHost mysql-agy-digitalocean-2\n  HostName 5.6.7.8\n")]
    (is (nil? (ssh-config/foreign-stanza-line lines "mysql-agy-digitalocean")))
    (is (= 4 (ssh-config/foreign-stanza-line lines "mysql-agy-digitalocean-2")))))
...
(deftest our-own-managed-block-is-not-foreign-for-any-alias-in-it
  ...
  (let [lines (str/split-lines
               (str "# BEGIN mysql-agy-digitalocean ANSIBLE MANAGED BLOCK\n"
                    "Host mysql-agy-digitalocean\n  HostName 1.2.3.4\n"
                    "Host mysql-agy-digitalocean-0\n  HostName 1.2.3.4\n"
                    "Host mysql-agy-digitalocean-1\n  HostName 1.2.3.5\n"
                    "Host mysql-agy-digitalocean-2\n  HostName 1.2.3.6\n"
                    "# END mysql-agy-digitalocean ANSIBLE MANAGED BLOCK\n"))
        marker "mysql-agy-digitalocean"]
...
(deftest a-member-stanza-outside-our-block-is-still-foreign
  (let [lines (str/split-lines
               (str "# BEGIN mysql-agy-digitalocean ANSIBLE MANAGED BLOCK\n"
                    ...
                    "# END mysql-agy-digitalocean ANSIBLE MANAGED BLOCK\n"
                    "Host mysql-agy-digitalocean-1\n  HostName 9.9.9.9\n"))]
    (is (= 5 (ssh-config/foreign-stanza-line lines "mysql-agy-digitalocean-1" "mysql-agy-digitalocean")))))
