...
# MYSQL_AGY_LIB_ROOT overrides), while green, red, and blue stay on their
...
build_variant() {
  ...
  (cd "$root/blue" && env COLORS_PAR_WORKDIR="$tmp/$variant/blue" COLORS_PAR_PROVIDER_BACKEND="$backend" \
    uv run python -m package_mysql_agy_blue build -f "$state" >/dev/null)
...
diff -r --exclude=__pycache__ "$root/green/src/resources/io/github/getcolors/mysql_agy" "$root/red/resources"
diff -r --exclude=__pycache__ "$root/green/src/resources/io/github/getcolors/mysql_agy" "$root/blue/src/package_mysql_agy_blue/resources"

echo "green, red, and blue mysql-agy artifacts are byte-identical"
