...
(def ^:private site
  {:path "skills/package-mysql-agy-green/green"
   :rx #"\(def \^:private mysql-agy-sha (nil|\"[0-9a-f]{40}\")\)"})
  ...
  (let [file (io/file (:path site))]
    (cond
      ...
      (let [text (slurp file)
        ...
        (if-not current
          {:green/exit 2
           :green/err (str "could not locate mysql-agy-sha in " (:path site))}
          (let [[head err] (repo-head "." "mysql-agy")]
            (cond
