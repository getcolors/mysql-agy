...
import {mysqlAgyWorkflow} from '../src/workflow.ts';
...
test('native SDK build renders one shared stack plus three nodes without SSH files',async()=>{
 const dir=mkdtempSync(join(tmpdir(),'mysql-agy-build-'));try{
  const opts={...fixture(),workdir:dir,'red/event':'build'};
  const result=await run(mysqlAgyWorkflow,opts);expect(result['red/exit']).toBe(0);
  ...
  const endpoint=readFileSync(join(files,'mysql-agy-endpoint'),'utf8');expect(endpoint).toContain('is_primary || exit 0');expect(endpoint).toContain('SELECT @@super_read_only');expect(endpoint).toContain('--node-id "$NODE_UID"');expect(endpoint).not.toContain('api.digitalocean.com');
...
test('inventory readers refuse unknown or partial state and propagate destruction',async()=>{
 ...
 expect((await tools.loadInfrastructureStep(opts,async()=>({status:'destroyed'}) as any))['mysql-agy/already-destroyed']).toBe(true);
