...
describe("tools", () => {
  ...
  test("load-infrastructure adopts the state preflight handed on", async () => {
    ...
    const load = (event: string, state: unknown) =>
      tools.loadInfrastructureStep(fixture({ "red/event": event, "mysql-agy/state": state }));
    // A recorded cluster.
    let r = await load("delete", { params });
    expect(r["red/exit"]).toBe(0);
    expect(r["once/cluster"]).toEqual(params);
    expect(r["mysql-agy/infrastructure-present?"]).toBe(true);
    expect("mysql-agy/state" in r).toBe(false);
    ...
    expect(r["red/exit"]).toBe(0);
    expect(r["mysql-agy/infrastructure-present?"]).toBe(false);
  ...
  test("a real create resolves the cluster from the apply", () => {
    ...
    const apply = (p: unknown) => tools.resolveInfrastructure(opts, {
      ...opts, "red/exit": 0, ...(p === undefined ? {} : { "mysql-agy/outputs": { params: p } }),
    });
    let r = apply(params);
    expect(r["red/exit"]).toBe(0);
    expect(r["once/cluster"]).toEqual(params);
  ...
  test("the inventory names both groups", () => {
    ...
    expect(built.all.children.mysql.hosts["fixture-node-2"].ansible_ssh_private_key_file)
      .toBe("/home/build-placeholder/.ssh/mysql-agy-fixture");
  ...
  test("the local stage writes one block per alias and carries no address", () => {
    ...
    expect(data["ssh-config-identity-file"]).toBe("~/.ssh/mysql-agy-fixture");
    expect(data["host-alias"]).toBe("mysql-agy-fixture");
    ...
    expect(tools.sshConfigHosts(fixture())).toEqual([
      { name: "mysql-agy-fixture", ip: "192.0.2.11" },
      { name: "mysql-agy-fixture-0", ip: "192.0.2.11" },
      { name: "mysql-agy-fixture-1", ip: "192.0.2.12" },
      { name: "mysql-agy-fixture-2", ip: "192.0.2.13" },
    ]);
  });

  test("a delete whose state records no cluster has no block to withdraw", async () => {
    const r = await tools.ansibleLocalStep(
      fixture({ "red/event": "delete", "mysql-agy/infrastructure-present?": false }));
    expect(r["red/exit"]).toBe(0);
  });
  ...
  test("the rendered tree is exactly what a member needs", () => {
    ...
    for (const expected of [
      ...
      "mysql-agy-binlog-upload", "mysql-agy-restore-check", "mysql-agy-health",
    ]) {
  ...
  test("the source lists reach the template as JSON lists", () => {
    ...
    expect(data["digitalocean-ssh-sources-json"]).toBe('["203.0.113.10/32"]');
...
describe("workflow", () => {
  test("create forks after the local ssh config and joins at the cluster", () => {
    ...
    expect(nexts("mysql-agy/infrastructure", create)).toEqual(["mysql-agy/ansible-local"]);
    expect(workflow.wireFn("mysql-agy/ansible-local", create)?.[0]).toBe(tools.ansibleLocalStep);
    expect(nexts("mysql-agy/ansible-local", create)).toEqual(["mysql-agy/dns", "mysql-agy/base"]);
  ...
  test("build walks the same graph as create", () => {
    for (const step of ["mysql-agy/start", "mysql-agy/infrastructure", "mysql-agy/ansible-local",
  ...
  test("delete reads state first and destroys in reverse", () => {
    ...
    // (ssh-config.md §4).
    expect(nexts("mysql-agy/cleanup", del)).toEqual(["mysql-agy/ansible-local"]);
    expect(nexts("mysql-agy/ansible-local", del)).toEqual(["mysql-agy/dns"]);
    expect(nexts("mysql-agy/dns", del)).toEqual(["mysql-agy/infrastructure"]);
    expect(nexts("mysql-agy/infrastructure", del)).toEqual(["mysql-agy/ssh-cleanup"]);
    expect(workflow.wireFn("mysql-agy/ssh-cleanup", del)?.[0]).toBe(ssh.cleanupStep);
    expect(nexts("mysql-agy/ssh-cleanup", del)).toEqual([]);
  });
  ...
  test("a build fills the placeholder key paths", async () => {
    ...
    expect(r["red/exit"]).toBe(0);
    expect(r["ssh-private-key-path"]).toBe("/home/build-placeholder/.ssh/mysql-agy-fixture");
  ...
  test("health changes nothing", () => {
    ...
    for (const fn of [workflow.wireFn("mysql-agy/load-infrastructure", health)?.[0],
                      workflow.wireFn("mysql-agy/health", health)?.[0]]) {
  ...
  test("a real create on a fresh work directory reports the credentials, not a crash", async () => {
    ...
    const work = mkdtempSync(join(tmpdir(), "mysql-agy-red-fresh"));
    try {
  ...
  test("an unreadable backend fails a real delete closed", async () => {
    ...
    const r = await startUnreadable(
      fixture({ "red/event": "delete", "compute-prevent-destroy": false, ...credentials }));
    expect(r["red/exit"]).toBe(0);
    expect(r["mysql-agy/state"]).toEqual({ error: "tofu output failed: no backend" });
  ...
  test("a real delete adopts the recorded cluster", async () => {
    const r = await start(
      ...
      converged());
    expect(r["red/exit"]).toBe(0);
    expect(r["mysql-agy/state"]).toEqual({ params: converged() });
    const loaded = await tools.loadInfrastructureStep(r);
    ...
    expect(empty["mysql-agy/infrastructure-present?"]).toBe(false);
  });
  ...
  test("a whole build renders every stage", async () => {
    ...
    for (const stage of ["mysql-agy-infrastructure", "mysql-agy-ansible-local", "mysql-agy-dns", "mysql-agy-ansible"]) {
...
describe("ssh", () => {
  test("a build never names the operator's home", () => {
    ...
    expect(opts["ssh-private-key-path"]).toBe("/home/build-placeholder/.ssh/mysql-agy-fixture");
    expect(opts["ssh-public-key-path"]).toBe("/home/build-placeholder/.ssh/mysql-agy-fixture.pub");
    // The placeholder lands on the provider's own machine-key key.
    expect(opts["digitalocean-ssh-keys"]).toBe("/home/build-placeholder/.ssh/mysql-agy-fixture.pub");
  ...
  test("real events render the real path", () => {
    ...
    expect(String(opts["ssh-private-key-path"]).endsWith("/.ssh/mysql-agy-fixture")).toBe(true);
  });
...
describe("ssh-config", () => {
  const opts = fixture({ profile: "mysql-agy-digitalocean" });

  test("the deployment claims one alias per member and the bare profile", () => {
    // `ssh mysql-agy-digitalocean` is what the standard promises; the numbered
    ...
    expect(sshConfig.aliases(opts)).toEqual(
      ["mysql-agy-digitalocean", "mysql-agy-digitalocean-0", "mysql-agy-digitalocean-1", "mysql-agy-digitalocean-2"]);
  });

  test("the identity file stays unexpanded", () => {
    expect(sshConfig.identityFile(opts)).toBe("~/.ssh/mysql-agy-digitalocean");
  });

  test("a foreign stanza is found for any alias, not just the first", () => {
    const lines = "Host something\n  HostName 1.2.3.4\n\nHost mysql-agy-digitalocean-2\n  HostName 5.6.7.8\n"
      .split("\n");
    expect(sshConfig.foreignStanzaLine(lines, "mysql-agy-digitalocean")).toBeUndefined();
    expect(sshConfig.foreignStanzaLine(lines, "mysql-agy-digitalocean-2")).toBe(4);
  });
  ...
  test("our own managed block is not foreign for any alias in it", () => {
    ...
    // `# BEGIN mysql-agy-digitalocean-0 …`, never find it, and refuse to
    // converge because of a block this package wrote itself.
    const lines = [
      "# BEGIN mysql-agy-digitalocean ANSIBLE MANAGED BLOCK",
      "Host mysql-agy-digitalocean", "  HostName 1.2.3.4",
      "Host mysql-agy-digitalocean-0", "  HostName 1.2.3.4",
      "Host mysql-agy-digitalocean-1", "  HostName 1.2.3.5",
      "Host mysql-agy-digitalocean-2", "  HostName 1.2.3.6",
      "# END mysql-agy-digitalocean ANSIBLE MANAGED BLOCK",
    ];
    for (const alias of sshConfig.aliases(opts)) {
      expect(sshConfig.foreignStanzaLine(lines, alias, "mysql-agy-digitalocean")).toBeUndefined();
    }
  });
  ...
  test("a member stanza outside our block is still foreign", () => {
    const lines = [
      ...
      "Host mysql-agy-digitalocean", "  HostName 1.2.3.4",
      "# END mysql-agy-digitalocean ANSIBLE MANAGED BLOCK",
      "Host mysql-agy-digitalocean-1", "  HostName 9.9.9.9",
    ];
    expect(sshConfig.foreignStanzaLine(lines, "mysql-agy-digitalocean-1", "mysql-agy-digitalocean")).toBe(5);
  });
