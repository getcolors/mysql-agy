...
export async function loadInfrastructureStep(opts:Opts,reader:typeof read_deployment=read_deployment):Promise<Opts>{
 ...
 const result:any=await reader(opts);if(result.status==='destroyed')return {...opts,'mysql-agy/already-destroyed':true,'red/exit':0};
 ...
 return {...opts,'colors-compute/cluster':result.cluster,'colors-compute/shared':result.shared??{},'mysql-agy/infrastructure-present?':true,...(result.key?.private_key_path?{'ssh-private-key-path':result.key.private_key_path}:{}),'red/exit':0};
}
