...
//
// Two OpenTofu stages: `mysql-agy-infrastructure` owns the droplets, the
// reserved IP and both firewalls; `mysql-agy-dns` owns the Cloudflare records.
...
export const ansibleLocalTool = "mysql-agy-ansible-local";
...
export function outputParams(result: Opts): computeCluster.ClusterParams | undefined {
  return computeCluster.outputParams({ "tofu/outputs": result["mysql-agy/outputs"] });
}
...
// `mysql-agy/state`, or a fresh read when nothing was. An unreadable backend and
...
export async function loadInfrastructureStep(
  ...
  const { "mysql-agy/state": handed, ...rest } = opts;
  const state = "mysql-agy/state" in opts
  ...
  if (failed(result)) return result;
  return { ...result, "mysql-agy/infrastructure-present?": present };
}
...
export async function ansibleLocalStep(opts: Opts): Promise<Opts> {
  ...
  if (isDelete && opts["mysql-agy/infrastructure-present?"] === false) {
...
export async function cleanupStep(opts: Opts): Promise<Opts> {
  ...
  return ansible.ansibleWithSpec(
    opts, ansibleConfig(opts, "cleanup.yml", "mysql-agy/cleanup-recap"),
    ansibleSpecs(opts));
}
